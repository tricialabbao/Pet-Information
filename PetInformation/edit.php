...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (!in_array($_FILES['image']['type'], $allowed)) {
            ...
            $newName = uniqid('pet_', true) . '.' . $ext;
            if (!is_dir('uploads/')) mkdir('uploads/', 0755, true);
            if (move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/' . $newName)) {
                if ($pet['image'] && file_exists('uploads/' . $pet['image'])) unlink('uploads/' . $pet['image']);
                $imageName = $newName;
            } else {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE pets SET name=?,species=?,breed=?,age=?,owner_name=?,description=?,image=? WHERE id=?");
        $stmt->execute([$values['name'],$values['species'],$values['breed'],$values['age'],$values['owner_name'],$values['description'],$imageName,$id]);
...
<head>
    ...
    <title>Edit <?= htmlspecialchars($pet['name']) ?> – PawRegistry</title>
...
.btn-update { background:#3b82f6; color:#fff; border:none; border-radius:50px; padding:.65rem 2.2rem; font-weight:800; }
.btn-update:hover { background:#1d4ed8; color:#fff; }
...
<div class="page-header">
    <div class="container">
        <h1><i class="bi bi-pencil-square me-2"></i>Edit Pet – <?= htmlspecialchars($pet['name']) ?></h1>
        <p class="mb-0 opacity-75">Update the information below and save.</p>
    </div>
</div>
    ...
    <div class="form-card">
        ...
        <form method="POST" enctype="multipart/form-data" novalidate>
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control <?= isset($errors['name'])?'is-invalid':'' ?>"
                           maxlength="100" value="<?= htmlspecialchars($values['name']) ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="breed" class="form-control <?= isset($errors['breed'])?'is-invalid':'' ?>"
                           maxlength="100" value="<?= htmlspecialchars($values['breed']) ?>">
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="owner_name" class="form-control <?= isset($errors['owner_name'])?'is-invalid':'' ?>"
                           maxlength="100" value="<?= htmlspecialchars($values['owner_name']) ?>">
                    <?php if (isset($errors['owner_name'])): ?><div class="invalid-feedback"><?= htmlspecialchars($errors['owner_name']) ?></div><?php endif; ?>
                </div>
                <div class="col-12">
                    <label class="form-label">Description</label>
                    <textarea name="description" rows="3" class="form-control"><?= htmlspecialchars($values['description']) ?></textarea>
                </div>
                <div class="col-12">
                    <label class="form-label">Pet Photo <span class="text-muted fw-normal">(leave blank to keep current)</span></label>
                    ...
                    <div class="img-preview-wrap" id="previewWrap">
                        <?php if ($pet['image'] && file_exists('uploads/' . $pet['image'])): ?>
                            <img src="uploads/<?= htmlspecialchars($pet['image']) ?>" alt="Current photo">
                        <?php else: ?>
            ...
            <div class="d-flex gap-3 justify-content-end">
                ...
                <button type="submit" class="btn btn-update"><i class="bi bi-check2-circle me-1"></i> Update Pet</button>
            </div>
        </form>
    </div>
</div>
